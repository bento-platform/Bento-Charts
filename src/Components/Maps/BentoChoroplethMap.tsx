...
const BentoChoroplethMap = (
  ...
  const calculateColor = useCallback(
    (v: number | undefined): string =>
      interpolator(((v ?? minYVal) - minYVal) / (maxYVal - minYVal)),
    [interpolator, minYVal, maxYVal]
  );

  const shapeStyle = useCallback((f: GeoJSONFeatureType): PathOptions => {
  ...
  } as LeafletEventHandlerFnMap), [onClick, categoryProp])
